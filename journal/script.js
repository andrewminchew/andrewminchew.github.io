...
outputContainer.addEventListener('click', (event) => {
  ...
  if (event.target.id === 'delete-entry') {
    // Get the timestamp of the parent element of the clicked delete button
    const timestamp = event.target.closest('.entry').querySelector('.timestamp').textContent;
    let itemsArray = localStorage.getItem('items') ? JSON.parse(localStorage.getItem('items')) : [];
    // Remove the entry with the matching timestamp from the items array
    itemsArray = itemsArray.filter(item => item.timestamp !== timestamp);
    localStorage.setItem('items', JSON.stringify(itemsArray));
  ...
  if (event.target.id === 'hide-entry') {
    // Get the timestamp of the parent element of the clicked hide button
    const timestamp = event.target.closest('.entry').querySelector('.timestamp').textContent;
    let itemsArray = localStorage.getItem('items') ? JSON.parse(localStorage.getItem('items')) : [];
    // Set the visibility of the entry with the matching timestamp to "hide"
  ...
  if (event.target.id === 'change-date') {
    // Get the timestamp of the parent element of the clicked date change button
    const timestamp = event.target.closest('.entry').querySelector('.timestamp').textContent;
    ...
    saveBtn.addEventListener('click', () => {
      ...
      let itemsArray = localStorage.getItem('items') ? JSON.parse(localStorage.getItem('items')) : [];
      // Update the timestamp of the entry with the matching timestamp
      itemsArray.forEach(function (item) {
        if (item.timestamp === timestamp) {
  ...
  if (itemsArray.length) {
    itemsArray.forEach(function (item) {
      if (item.visibility !== "hide") {
        ...
        itemElement.innerHTML = `<div class="text"><span class="timestamp">${item.timestamp}</span>${item.text}</div><div class="toolbar"><button id="change-date">change&nbsp;date</button><button id="delete-entry">delete</button><button id="hide-entry">hide</button>
        </div>`;
